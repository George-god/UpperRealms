...
class PvEBattleService
{
     ...
     * Enemy stats use the PvE scaling model by default. Dungeons pass $scaleNpcWithPlayerLevel = false
     * so stage HP/attack/defense from data are used as-is (those values are already tuned per dungeon).
     */
    public function simulateCustomBattle(
        ...
        bool $scaleNpcWithPlayerLevel = true
    ): array {
        try {
            $statCalc = new StatCalculator();
            $userStats = $statCalc->calculateFinalStats($userId);
            ...
            $scaleLevel = $scaleNpcWithPlayerLevel ? $playerLevel : 0;
