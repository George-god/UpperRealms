...
class DungeonService
{
    ...
    public function advanceRun(int $userId, int $runId): array
    {
        ...
        try {
            ...
            if ($won) {
                ...
                if ($isBossStage) {
                    try {
                        ...
                        error_log('DungeonService::advanceRun grantBossRewards ' . $e->getMessage());
                        $rewards = [
                            'gold' => 0,
                            'spirit_stones' => 0,
                            'manual' => null,
                    ...
                    $manualId = null;
                    if (isset($rewards['manual']) && is_array($rewards['manual'])) {
                        $manualId = isset($rewards['manual']['id']) ? (int)$rewards['manual']['id'] : null;
                    }
                    DaoRecord::log(
                        'dungeon_run',
                        $userId,
            ...
            error_log('DungeonService::advanceRun ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
            $out = [
                'success' => false,
                'message' => 'Could not save dungeon progress after the battle. You can try this stage again; if it keeps failing, check the server log.',
            ];
            if ($payload !== null && $stage !== null) {
                $out['battle'] = $payload;
                $out['stage_name'] = $stage['label'] ?? 'Battle';
                $out['message'] = 'Progress could not be saved, but the fight outcome is shown below. Winner: '
                    . ($payload['winner'] === 'user' ? 'You' : 'Enemy')
