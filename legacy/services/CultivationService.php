...
require_once __DIR__ . '/RealmService.php';
 ...
 * Level cannot pass the realm cap (next realm's required_level, or final realm max_level) until breakthrough.
 */
class CultivationService
{
    ...
    public function cultivate(int $userId): array
    {
        try {
            ...
            $levelCap = (new RealmService())->getCultivationLevelCap($userId);
            $levelUpResult = $this->tryLevelUp($db, $userId, $newChi, $maxChi, $level, $newChi, $levelCap);
     ...
     */
    private function tryLevelUp(\PDO $db, int $userId, int $currentChi, int $maxChi, int $level, int $chiAfterCultivate, int $levelCap): array
    {
